...
import {
  OrganizoNavigationItem,
  OrganizoVerticalNavigationAppearance,
  OrganizoVerticalNavigationMode,
  OrganizoVerticalNavigationPosition
} from "@organizo/components/navigation/navigation.types";
...
import {OrganizoScrollbarDirective} from "@organizo/directives/scrollbar/scrollbar.directive";
...
@Component({
  ...
  standalone: true,
  imports: [OrganizoScrollbarDirective, OrganizoVerticalNavigationAsideItemComponent, OrganizoVerticalNavigationBasicItemComponent, OrganizoVerticalNavigationCollapsableItemComponent, OrganizoVerticalNavigationDividerItemComponent, OrganizoVerticalNavigationGroupItemComponent, OrganizoVerticalNavigationSpacerItemComponent],
})
export class OrganizoVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy {
  ...
  @Input() appearance: OrganizoVerticalNavigationAppearance = 'default';
  @Input() autoCollapse: boolean = true;
  @Input() inner: boolean = false;
  @Input() mode: OrganizoVerticalNavigationMode = 'side';
  @Input() name: string = this._organizoUtilsService.randomId();
  @Input() navigation: OrganizoNavigationItem[];
  @Input() opened: boolean = true;
  @Input() position: OrganizoVerticalNavigationPosition = 'left';
  @Input() transparentOverlay: boolean = false;
  @Output() readonly appearanceChanged: EventEmitter<OrganizoVerticalNavigationAppearance> = new EventEmitter<OrganizoVerticalNavigationAppearance>();
  @Output() readonly modeChanged: EventEmitter<OrganizoVerticalNavigationMode> = new EventEmitter<OrganizoVerticalNavigationMode>();
  @Output() readonly openedChanged: EventEmitter<boolean> = new EventEmitter<boolean>();
  @Output() readonly positionChanged: EventEmitter<OrganizoVerticalNavigationPosition> = new EventEmitter<OrganizoVerticalNavigationPosition>();
  activeAsideItemId: string | null = null;
  onCollapsableItemCollapsed: ReplaySubject<OrganizoNavigationItem> = new ReplaySubject<OrganizoNavigationItem>(1);
  onCollapsableItemExpanded: ReplaySubject<OrganizoNavigationItem> = new ReplaySubject<OrganizoNavigationItem>(1);
  ...
  private _organizoScrollbarDirectivesSubscription: Subscription;
  ...
  private _organizoScrollbarDirectives!: QueryList<OrganizoScrollbarDirective>;
  ...
  /**
   * Setter for organizoScrollbarDirectives
   */
  @ViewChildren(OrganizoScrollbarDirective)
  set organizoScrollbarDirectives(organizoScrollbarDirectives: QueryList<OrganizoScrollbarDirective>) {
    // Store the directives
    this._organizoScrollbarDirectives = organizoScrollbarDirectives;

    // Return if there are no directives
    if (organizoScrollbarDirectives.length === 0) {
      return;
    }

    // Unsubscribe the previous subscriptions
    if (this._organizoScrollbarDirectivesSubscription) {
      this._organizoScrollbarDirectivesSubscription.unsubscribe();
    }

    // Update the scrollbars on collapsable items' collapse/expand
    this._organizoScrollbarDirectivesSubscription =
      merge(
        ...
        .subscribe(() => {
          ...
          organizoScrollbarDirectives.forEach((organizoScrollbarDirective) => {
            organizoScrollbarDirective.update();
          });
        });
  }
  ...
  @HostBinding('class') get classList(): any {
    /* eslint-disable @typescript-eslint/naming-convention */
    return {
      'organizo-vertical-navigation-animations-enabled': this._animationsEnabled,
      [`organizo-vertical-navigation-appearance-${this.appearance}`]: true,
      'organizo-vertical-navigation-hover': this._hovered,
      'organizo-vertical-navigation-inner': this.inner,
      'organizo-vertical-navigation-mode-over': this.mode === 'over',
      'organizo-vertical-navigation-mode-side': this.mode === 'side',
      'organizo-vertical-navigation-opened': this.opened,
      'organizo-vertical-navigation-position-left': this.position === 'left',
      'organizo-vertical-navigation-position-right': this.position === 'right',
    };
    /* eslint-enable @typescript-eslint/naming-convention */
  }
  ...
  ngAfterViewInit(): void {
    ...
    setTimeout(() => {
      ...
      if (!this._navigationContentEl.nativeElement.classList.contains('ps')) {
        ...
        const activeItem = this._navigationContentEl.nativeElement.querySelector('.organizo-vertical-navigation-item-active');
      ...
      // Otherwise
        ...
        this._organizoScrollbarDirectives.forEach((organizoScrollbarDirective) => {
          // Skip if not enabled
          if (!organizoScrollbarDirective.isEnabled()) {
            return;
          }

          // Scroll to the active element
          organizoScrollbarDirective.scrollToElement('.organizo-vertical-navigation-item-active', -120, true);
        });
      }
    });
  }
   ...
   * @param item
   */
  openAside(item: OrganizoNavigationItem): void {
   ...
   * @param item
   */
  toggleAside(item: OrganizoNavigationItem): void {
    // Toggle
  ...
  private _showOverlay(): void {
    ...
    this._overlay.classList.add('organizo-vertical-navigation-overlay');

    // Add a class depending on the transparentOverlay option
    if (this.transparentOverlay) {
      this._overlay.classList.add('organizo-vertical-navigation-overlay-transparent');
    }
  ...
  private _showAsideOverlay(): void {
    ...
    this._asideOverlay.classList.add('organizo-vertical-navigation-aside-overlay');
