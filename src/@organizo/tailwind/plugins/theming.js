...
// -----------------------------------------------------------------------------------------------------
// @ ORGANIZO TailwindCSS Main Plugin
// -----------------------------------------------------------------------------------------------------
const theming = plugin.withOptions((options) => ({
    ...
    addComponents(
      _.fromPairs(_.map(options.themes, (theme, themeName) => [
        ...
        ))), (value, key) => [[`--organizo-${e(key)}`, value], [`--organizo-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
      ])),
    );
    ...
    const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) => {
      ...
      const background = theme(`organizo.customProps.background.${colorScheme}`);
      const foreground = theme(`organizo.customProps.foreground.${colorScheme}`);
      ...
      return {
        [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
          ...
          ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--organizo-${e(key)}`, value], [`--organizo-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
          ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--organizo-${e(key)}`, value], [`--organizo-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
        },
      };
    });
      ...
      theme: {
        extend: {
          ...
          colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
            [name, `rgba(var(--organizo-${name}-rgb), <alpha-value>)`],
            [`on-${name}`, `rgba(var(--organizo-on-${name}-rgb), <alpha-value>)`],
          ]))),
        },
        organizo: {
          customProps: {
